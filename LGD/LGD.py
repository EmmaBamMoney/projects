...
mydb = client["bam"]
mycol = mydb["assets"]
df = pd.DataFrame(list(mycol.find({"_id" : ObjectId("5bb7eb2ae815641f3416415f")})))

mongostore = df.apply(lambda x: 100-individual_recovery_rate(x), axis=1).values
mongostore = zip(df['_id'], mongostore)
